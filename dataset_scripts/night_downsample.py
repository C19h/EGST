...
class Downsample():
    ...
    def exe(self):
        if not os.path.exists(self.Config.downsample_dir):
            origin_path_list = []
            target_path_list = []
            for test_train in os.listdir(self.Config.origin_data_dir):
                # iterate each person
                for person in os.listdir(os.path.join(self.Config.origin_data_dir, test_train)):
                    # make corresponding person graph dir
                    train_samples, test_samples = train_test_split(
                        os.listdir(os.path.join(self.Config.origin_data_dir, test_train, person)),
                        test_size=0.5, random_state=0)
                    for file in train_samples:
                        target_path_list.append(
                            os.path.join(self.Config.downsample_dir, 'train', person, file.replace('txt', 'mat')))
                        if not os.path.exists(os.path.join(self.Config.downsample_dir, 'train', person)):
                            os.makedirs(os.path.join(self.Config.downsample_dir, 'train', person))
                        origin_path_list.append(os.path.join(self.Config.origin_data_dir, test_train, person, file))
                    for file in test_samples:
                        target_path_list.append(
                            os.path.join(self.Config.downsample_dir, 'test', person, file.replace('txt', 'mat')))
                        if not os.path.exists(os.path.join(self.Config.downsample_dir, 'test', person)):
                            os.makedirs(os.path.join(self.Config.downsample_dir, 'test', person))
                        origin_path_list.append(os.path.join(self.Config.origin_data_dir, test_train, person, file))
...
if __name__ == '__main__':
    ...
    origin_path_list = []
    target_path_list = []
    for test_train in os.listdir(Config.origin_data_dir):
        # iterate each person
        for person in os.listdir(os.path.join(Config.origin_data_dir, test_train)):
            # make corresponding person graph dir
            train_samples, test_samples = train_test_split(
                os.listdir(os.path.join(Config.origin_data_dir, test_train, person)),
                test_size=0.5, random_state=0)
            for file in train_samples:
                target_path_list.append(
                    os.path.join(Config.downsample_dir, 'train', person, file.replace('txt', 'mat')))
                if not os.path.exists(os.path.join(Config.downsample_dir, 'train', person)):
                    os.makedirs(os.path.join(Config.downsample_dir, 'train', person))
                origin_path_list.append(os.path.join(Config.origin_data_dir, test_train, person, file))
            for file in test_samples:
                target_path_list.append(os.path.join(Config.downsample_dir, 'test', person, file.replace('txt', 'mat')))
                if not os.path.exists(os.path.join(Config.downsample_dir, 'test', person)):
                    os.makedirs(os.path.join(Config.downsample_dir, 'test', person))
                origin_path_list.append(os.path.join(Config.origin_data_dir, test_train, person, file))
