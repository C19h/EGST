...
class ApiDoc(object):
    ...
    def init_app(self, app, title="Api Doc", version="1.0.0", description=""):
        ...
        with app.app_context():
            ...
            def data():
                ...
                router_list = current_app.config["API_DOC_ROUTER_NAME"]
                order = []
                for row in router_list:
    ...
    def _get_api_data(self):
        ...
        router_list = current_app.config["API_DOC_ROUTER_NAME"]
        router_dict = {x[0]:x[1] for x in router_list}
        data_dict = {}
        ...
        for rule in current_app.url_map.iter_rules():
            ...
            if bp_name not in router_dict:
                continue

            if member_sub_name in current_app.config["API_DOC_MEMBER_SUB_EXCLUDE"]:
                continue

            if not router_dict is None and bp_name in router_dict:
                bp_name = router_dict[bp_name]
    ...
    def _get_doc_name_extra_doc_md(self, doc_src):
        ...
        if len(doc_src.split("\n", 1)) >= 2:
            doc_md = doc_src.split("\n", 1)[1].strip(" ")
            tstr = doc_src.split("\n", 1)[1].split("\n")[0]
            space_count = 0
            for c in tstr:
                if c == ' ':
